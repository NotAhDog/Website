...
def all_folios():
    ...
    return render_template('folios.html', folios=folios, title=title,
                           folio_num=len(folios), login=GetLogin())
...
def login():
    ...
    if request.method == "GET":
        ...
        if account and check_password_hash(account.password, password):
            ...
            return redirect(url_for('root'))  # or wherever
        else:
            error = True

    return render_template('login.html', form=form, title=title,
                           login=GetLogin(), error=error)
...
def signup():
    ...
    if request.method == "GET":
        ...
        if form.validate_on_submit():
            ...
            if existing_user:
                ...
                return redirect(url_for('root'))  # Redirect to login page
        else:  # a validator failed... lets see which one(s)
            # print('-------------------------------')
            # print('ERROR:')
            # print(form.errors)
    ...
    return render_template('signup.html', form=form, title=title,
                           login=GetLogin(), errors=errors)


@app.route('/dashboard', methods=['GET', 'POST'])
def dashboard():
    user_id = models.User.query.filter_by(username=session.get
                                          ('username')).first().id
    print(user_id, "uid")
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if form.validate_on_submit():
                ...
                # print(form.errors)
                # print('-------------------------------')
                submittedforms = Feedback.query.filter_by(
                                    user_id=user_id).all()
